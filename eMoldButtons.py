...
def draw_grid(arg): # ебаный рот подсмешок, де коменты сука, давно реактивный пе-8 не выкачивал? # МАЯ ТЫКАТЬ КНОПКИ, МАЯ НЕ УМЕТЬ ПИСАТЬ БУКВА
    block_size, WINDOW_WIDTH, WINDOW_HEIGHT, BLACK = arg # Получаем все аргументы
    for x in range(0, WINDOW_WIDTH, block_size): # Проходимся по x и y вплоть до ширины поля с шагом в величину блоков
        for y in range(0, WINDOW_HEIGHT, block_size): # тут тоже
            rect = pygame.Rect(x, y, block_size, block_size) # Задаём квадратик

            pygame.draw.rect(SCREEN, BLACK, rect, 1) # Рисуем квадратик
...
pygame.init()  # вот это должно быть в сетапе # Если поместить, то оно залагает и умрёт
SCREEN = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
CLOCK = pygame.time.Clock()  # вот это время в чем то как то ну там спрашивайте ПодСмешка # Зачем тут этот комментарий, если он не несёт НИКАКОГО СМЫСЛА!
SCREEN.fill(WHITE)
